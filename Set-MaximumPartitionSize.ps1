...
if ($NumBlockDevices -eq $NumDiskpartDisks) {
    ...
    Write-Output "Some drives missing!" | Out-File $LogFile -Append
    $DrivesMissing = $true
}
...
# Drives missing? Likely a VDS service crash (RPC failure reported). Test with more than 5 EBS + ephemeral drives.
# Observed only after rapid multiple script executions. If otherwise reproducible, here are some thoughts:
# [1] Add "Start-Sleep 3" below line 86. Quick and dirty.
# [2] Do not resize drives that are already at max size. Much better option.
if ($DrivesMissing) {
    ...
    Get-EventLog System | Where-Object {$_.EventID -eq '7031'} | Select-Object EntryType, EventID, TimeGenerated, Source, Message | Format-Table -AutoSize -Wrap | Out-File $LogFile -Append
}
